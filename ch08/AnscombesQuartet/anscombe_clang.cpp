...
int main() {
   ...
   std::cout << fmt::format("{:>7}  {:>9}  {:>7}\n", 
      "Dataset", "Intercept", "Slope");
   for (int i{0}; i < 4; ++i) {
      std::cout << fmt::format("{:>7}  {:>9.5f}  {:>7.5f}\n", 
   ...
   for (int i{0}; i < 4; ++i) {
      ...
      pipe.sendLine(fmt::format("set output '{}dataset{}.png'", 
         location, i + 1));

      // gnuplot's "set title" command sets the title above the plot
      pipe.sendLine(fmt::format("set title 'Dataset {}'", i + 1));
      ...
      std::string plotCommand{
         fmt::format("plot '{}anscombe.csv' ", location) + 
         fmt::format("using {}:{} ", i * 2 + 1, i * 2 + 2) +
         "with points "s + 
         "pointtype 7, "s +
         fmt::format("{:f}*x+{:f} ", slopes[i], intercepts[i]) + 
